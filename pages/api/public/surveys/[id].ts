...
async function PublicSurveyEndpoint(req: RequestWithPrisma, res: NextApiResponse) {
  switch (req.method) {
    case 'GET':
      try {
    ...
    case 'POST':
      try {
        ...
        const surveysLength = await req.db.survey.count({
          where: {
            id,
          },
        })
...
export default withPrisma(PublicSurveyEndpoint)
