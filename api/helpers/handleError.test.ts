...
describe('api/helpers/handleError()', () => {
  ...
  test('with a TooCustomError error', () => {
    ...
    expect(B.error).toHaveBeenNthCalledWith(
      ...
      expect.stringContaining(
        `[api/helpers/handleError()] This type of error can't be processed. This should never happen.`,
      ),
    )
    expect(B.error).toHaveBeenNthCalledWith(
      2,
      expect.stringContaining(`[api/helpers/handleError()] Error Type: object`),
    )
    expect(B.error).toHaveBeenNthCalledWith(
      3,
      expect.stringContaining(`[api/helpers/handleError()] Error Constructor: TooCustomError`),
    )
  ...
  test('with an undefined error', () => {
    ...
    expect(B.error).toHaveBeenNthCalledWith(
      2,
      expect.stringContaining(`[api/helpers/handleError()] Error Type: undefined`),
    )
    expect(B.error).toHaveBeenNthCalledWith(
      3,
      expect.stringContaining(`[api/helpers/handleError()] Error Constructor: undefined`),
    )
