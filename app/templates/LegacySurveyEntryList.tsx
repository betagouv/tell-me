...
export default function LegacySurveyEntryList() {
  ...
  const exportEntries = async fileExtension => {
    ...
    window.open(`/api/legacy/survey/${surveyId}/download?fileExtension=${fileExtension}&oneTimeToken=${oneTimeToken}`)
  ...
  const loadSurvey = async () => {
    const maybeBody = await api.get(`legacy/survey/${surveyId}`)
    if (maybeBody === null || maybeBody.hasError) {
      return
    }
  ...
  const loadSurveyEntries = async () => {
    const maybeBody = await api.get(`legacy/survey/${surveyId}/entries`)
    if (maybeBody === null || maybeBody.hasError) {
      return
    }
