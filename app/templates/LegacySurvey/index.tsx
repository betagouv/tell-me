...
import LegacySurveyManager from '../../libs/LegacySurveyManager'
...
export default function LegacySurvey({ data: survey }) {
  ...
  const submitSurvey = async (values, { setSubmitting }) => {
    ...
    const maybeBody = await api.post('legacy/survey/entry', surveyEntry)
    ...
    await Promise.all(
      fileAnswers.map(async fileAnswer => {
        ...
        await api.put(
          `legacy/survey/entry-upload?surveyEntryId=${surveyEntryId}&type=${type}&question=${question}`,
          formData,
        )
      }),
    )
